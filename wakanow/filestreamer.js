	...
	validatefileexists : function(filename) {
		return fs.exists(filename, function(exists) {
			if (exists) {
				...
				wakanow.logtoFile("Test data file NOT found @" + filename);
				driver.quit();
			}
		});
	},
        readDataFromCSV : function(filePath ,key){
			var fstream = fs.createReadStream(filePath).on('err', function(err) {
        ...
        parser.on('end', function() {
            ...
            console.log('total records :: '+ data.address.lga_csv.total_records);
            return false;
